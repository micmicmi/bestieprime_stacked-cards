...
# Read stacked cards (the original 3D animation)
with open('stacked cards.html', 'r', encoding='utf-8') as f:
    stacked_html = f.read()

head_match = re.search(r'(<head>.*?</head>)', stacked_html, re.DOTALL)
body_match = re.search(r'(<div class="mwg_effect031">.*?)<script>', stacked_html, re.DOTALL)
script_match = re.search(r'(<script>.*?</script>.*?</body>)', stacked_html, re.DOTALL)

original_head = head_match.group(1) if head_match else ""
original_cards = body_match.group(1) if body_match else ""
original_script = script_match.group(1) if script_match else ""

# Decrease height by 30%: replace 100vh with 70vh in original_head CSS
original_head = original_head.replace('height: 100vh;', 'height: 70vh;')
original_head = original_head.replace('height: 100dvh;', 'height: 70dvh;')

# Adjust pinning duration in script to 70vh
original_script = original_script.replace('window.innerHeight', 'window.innerHeight * 0.7')
...
final_head = original_head.replace('</style>', new_styles + '</style>')
...
print("Updated test.html with correct animation and layout.")
